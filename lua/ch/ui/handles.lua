...
function model:init()
  local _items = ch.handle
  ---@type table<string, { expand: boolean, items: ch.types.handle[]}[]>
  local items = {}
  ...
  api.nvim_set_hl(0, 'UIEvent', { fg = ch.lib.hl:get('syntax', 'event') })
...
local fmt = function(tpe, ...)
  local fmts = {
    event = ch.lib.icons.syntax.event..' %s',
    priority = function(priority)
      return (ch.lib.icons.info.loaded .. ' %s'):format(priority)
    end,
    task = function(desc)
      return (ch.lib.icons.info.loaded .. ' %s'):format(desc or ch.lib.icons.syntax.fn)
    end,
  }
  local f = fmts[tpe]
  if not f then

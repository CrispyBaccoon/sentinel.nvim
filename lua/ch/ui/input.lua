-- adapted from @dressing.nvim https://github.com/stevearc/dressing.nvim/blob/8b7ae53d7f04f33be3439a441db8071c96092d19/lua/dressing/input.lua

local global_config = ch.modules.ch.ui.opts.input
local util = require 'ch.ui.util'

local M = {}

---@class InputContext
---@field opts? InputOptions
...
---@param opts string|InputOptions
...
local function show_input(opts, on_confirm)
  ...
  for mode, user_maps in pairs(config.mappings) do
    for action, lhs in pairs(user_maps) do
      local map = mappings[action]
      if map then
        vim.keymap.set(mode, lhs, map.rhs, { buffer = bufnr, desc = map.desc })
      end
    end
  end
